...
class QEC_d3(Quantum_Operation):
    ...
    def run_fullQEC_surface17_allstabs_min_sequential(self, code, bare=True):
        '''
        Runs the minimal meta-decoder (1 or 2 rounds) for the 
        surface-17 code with all stabs in the same round.
        Meta-decoder: if no error is detected in round 1: stop.
                      else: do one more and keep last syndrome.

        At the end, it applies a correction.
        It returns the number of QEC rounds.
        
        We don't have Identity blocks in between the stabilizer blocks.
        '''
        if code == 'surface17':  
            code_class = surf17.Code
        elif code == 'BS17':
            code_class = BS17.Code

        QEC_func = self.run_one_bare_anc_new
        ...
        data_errors += [QEC_func(1, code, 'both')]
        #print 'errors =', data_errors
        
        if code == 'surface17':
            # run second round if errors were detected 
            if sum(data_errors[0]) != 0:

                # run third subcircuit (Identity)
                # second_I_outcome = self.run_one_circ(2)
                # second round of stabilizers
                data_errors += [QEC_func(2, code, 'both')]

        elif code == 'BS17':
            X_syns_BS = [data_errors[0][0], data_errors[0][2], data_errors[0][5], data_errors[0][7]]
            Z_syns_BS = [data_errors[0][1], data_errors[0][3], data_errors[0][4], data_errors[0][6]]

            X_syns = [(X_syns_BS[1]+X_syns_BS[3])%2, (X_syns_BS[0]+X_syns_BS[2])%2]
            Z_syns = [(Z_syns_BS[0]+Z_syns_BS[1])%2, (Z_syns_BS[2]+Z_syns_BS[3])%2]

            if (sum(X_syns)+sum(Z_syns)) != 0:
                # run second round if errors were detected
                data_errors += [QEC_func(2, code, 'both')]


        # run (second) or third Identity round
        last_I_outcome = self.run_one_circ(3)


        # we keep the last round
        correct_syns = data_errors[-1]
        #print data_errors
        #print correct_syns
        
        if code == 'surface17':
            X_syns = [correct_syns[0], correct_syns[2], correct_syns[5], correct_syns[7]]
            Z_syns = [correct_syns[1], correct_syns[3], correct_syns[4], correct_syns[6]]


        elif code == 'BS17':
            X_syns_BS = [data_errors[-1][0], data_errors[-1][2], data_errors[-1][5], data_errors[-1][7]]
            Z_syns_BS = [data_errors[-1][1], data_errors[-1][3], data_errors[-1][4], data_errors[-1][6]]
        ...
        # first X stabs
        X_syns_string = ''.join(map(str,X_syns))
        qubits_corr = code_class.lookuptable['Xstabs'][X_syns_string]
        Z_errors = ['Z' if i in qubits_corr else 'I' for i in range(9)]
        
        # then Z stabs
        Z_syns_string = ''.join(map(str,Z_syns))
        qubits_corr = code_class.lookuptable['Zstabs'][Z_syns_string]
        X_errors = ['X' if i in qubits_corr else 'I' for i in range(9)]
        ...
        if 'Z' in Z_errors:
            #print 'stabs =', self.stabs
            #print 'Z_corr =', Z_corr
            Z_corr = pre_Is[:] + Z_errors[:] + post_Is[:]
            #print 'Z_corr =', Z_corr
            corr_state = qfun.update_stabs(self.stabs,
                                           self.destabs,
                                           Z_corr)
        ...
        if 'X' in X_errors:
            X_corr = pre_Is[:] + X_errors[:] + post_Is[:]
            corr_state = qfun.update_stabs(self.stabs,
                                           self.destabs,
                                           X_corr)
            #print 'X_corr =', X_corr
        ...
        return len(data_errors), X_syns, Z_syns, Z_errors, X_errors

    
    
    def run_prep_surface17_min_sequential(self, code, state_prep='Z'):
        '''
        Prepares a logical state on the surface-17 code with the 
        minimal meta-decoder (2 or 3 rounds)
        Meta-decoder:
        ...
        state_prep: X to prep + or Z to prep 0
        ...
        # Run first circuit (preparation of data qubits)
        first_I_outcome = self.run_one_circ(0)

        # Run first round of stabilizers (complete)
        data_errors += [QEC_func(1, code, 'both')]
        correct_syns = data_errors[0]
        X_syns1 = [correct_syns[0], correct_syns[2], correct_syns[5], correct_syns[7]]
        Z_syns1 = [correct_syns[1], correct_syns[3], correct_syns[4], correct_syns[6]]

        #print 'X stabs 1:', X_syns1
        #print 'Z stabs 1:', Z_syns1

        # if we want to prep a logical 0
        if state_prep == 'Z':

            # if the Z stabilizers detected an error, run both kinds of stabs again
            if sum(Z_syns1) != 0:
                
                #print 'X errors were detected'
                ...
                correct_syns = data_errors[1]
                X_syns2 = [correct_syns[0], correct_syns[2], correct_syns[5], correct_syns[7]]
                Z_syns2 = [correct_syns[1], correct_syns[3], correct_syns[4], correct_syns[6]]
                Z_syns_def = Z_syns2[:]

                #print 'X stabs 2:', X_syns2
                #print 'Z stabs 2:', Z_syns2
                

            # else, run only the other kind of stabs
            else:
                #print 'No X errors were detected, so no more Z stabs will be measured.'

                Z_syns_def = Z_syns1[:]
                
                data_errors += [QEC_func(3, code, 'both')]
                
                correct_syns = data_errors[1]
                X_syns2 = [correct_syns[0], correct_syns[2], correct_syns[5], correct_syns[7]]

                #print 'X stabs 2:', X_syns2
                #print 'Z stabs 2: not measured'

            # if the X stabilizers don't coincide, run them a third time
            if X_syns1 != X_syns2:

                #print 'X1 and X2 stabs dont coincide. Need to measure them a third time'

                data_errors += [QEC_func(4, code, 'both')]

                correct_syns = data_errors[2]
                X_syns3 = [correct_syns[0], correct_syns[2], correct_syns[5], correct_syns[7]]
                X_syns_def = X_syns3[:]

                #print 'X stabs 3:', X_syns3
                #print 'Z stabs 3: not measured'

            else:
                #print 'No Z errors were detected'

                X_syns_def = X_syns2[:]
                
                #print 'X stabs 3: not measured'
                #print 'Z stabs 3: not measured'


        elif state_prep == 'X':
            
            # if the X stabilizers detected an error, run both kinds of stabs again
            if sum(X_syns1) != 0:
                
                #print 'Z errors were detected'
            ...
            # else, run only the other kind of stabs
                ...
                #print 'No Z errors were detected, so no more X stabs will be measured.'
                ...
                #print 'X stabs 2: not measured'
                #print 'Z stabs 2:', Z_syns2


            # if the Z stabilizers don't coincide, run them a third time
            if Z_syns1 != Z_syns2:
                
                #print 'Z1 and Z2 stabs dont coincide. Need to measure them a third time'
                
                data_errors += [QEC_func(4, code, 'both')]

                correct_syns = data_errors[2]
                Z_syns3 = [correct_syns[1], correct_syns[3], correct_syns[4], correct_syns[6]]
                Z_syns_def = Z_syns3[:]
                
                #print 'X stabs 3: not measured'
                #print 'Z stabs 3:', Z_syns3

            else:
                #print 'No X errors were detected'
        ...
        # first X stabs
        X_syns_string = ''.join(map(str,X_syns_def))
        qubits_corr = code_class.lookuptable['Xstabs'][X_syns_string]
        Z_errors = ['Z' if i in qubits_corr else 'I' for i in range(9)]
        
        # then Z stabs
        Z_syns_string = ''.join(map(str,Z_syns_def))
        qubits_corr = code_class.lookuptable['Zstabs'][Z_syns_string]
        X_errors = ['X' if i in qubits_corr else 'I' for i in range(9)]
        ...
        return len(data_errors), X_syns_def, Z_syns_def, Z_errors, X_errors

    

    def run_prep_surface17_min_sequential_minimal(self, state_prep='Z'):
        '''
        Prepares a logical state on the surface-17 code with the absolutely 
        minimal meta-decoder (1 or 2 rounds)
        Meta-decoder:
        ...
        if state_prep == 'Z':
            ...
            X_syns_def = X_syns1[:]
            Z_errors = ['I' for i in range(9)]

            # if the Z stabilizers detected an error, run the Z stabilizers again
            if sum(Z_syns1) != 0:
            ...
            # if the Z stabilizers didn't detect an error, stop
            else:
                Z_syns_def = Z_syns1[:]
                X_errors = ['I' for i in range(9)]


        # if we want to prepare a logical +
        elif state_prep == 'X':
            
            Z_syns_def = Z_syns1[:]
            X_errors = ['I' for i in range(9)]

            # if the X stabilizers detected an error, run the X stabilizers again
            if sum(X_syns1) != 0:
            ...
            # if the X stabilizers didn't detect an error, stop
            else:
                X_syns_def = X_syns1[:]
                Z_errors = ['I' for i in range(9)]
    ...
    def run_fullQEC_CSS_perfect(self, code):
        '''
        runs 1 round of QEC for a CSS code,
        like the Steane code, the surface-17, or the Bacon-Shor.
        At the end, it applies a correction.
        It assumes there are two sub-circuits and 
        the X stabilizers come first.
        pre_n:  number of physical qubits before the physical
                qubits onto which this QEC is acting.
        post_n: number of physical qubits after the physical
                qubits onto which this QEC is acting.
        ...
        # run the 2 subcircuits (X stabs first)
        Z_data_errors += [QEC_func(0, code, 'X')[0]]
        X_data_errors += [QEC_func(1, code, 'Z')[0]]
