...
# The next lines are to generate the dat file for the logical preparation error rate (surface17)
list_prets = [(i/250. - 8) for i in range(1000)]
list_ts = [10**i for i in list_prets]


output_string = 't p_lowerLD p_upperLD p_lowerST p_upperST t_lowerLD t_upperLD t_lowerST t_upperST\n'
for t in list_ts:
    ...
    output_string += '%.15f %.15f %.15f %.15f %.15f %.15f %.15f %.15f %.15f\n' %(t, p_lower_LD, p_upper_LD, p_lower_ST, p_upper_ST, t_lower_LD, t_upper_LD, t_lower_ST, t_upper_ST) 

data_filename = summary_folder + 'resultsQECd3_Imeas' + str(int(ratio_meas_times)) + 'prep_surface17.dat'
