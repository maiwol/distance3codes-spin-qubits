...
# The next lines are to generate the dat file for several T2 times

list_prets = [(i/250. - 8) for i in range(1000)]
list_ts = [10**i for i in list_prets]

output_string = 't p_lowerST2 p_upperST2 p_lowerST3 p_upperST3 p_lowerST4 p_upperST4 p_lowerST0 p_upperST0 p_physST2_lower p_physST2_upper p_physST3_lower p_physST3_upper p_physST4_lower p_physST4_upper\n'
for t in list_ts:
    ...
    t_lower_ST2, t_lower_ST3, t_lower_ST4, t_lower_ST0 = 0., 0., 0., 0.
    ...
    p_data3 = 0.5*(1. - math.exp(-(0.1*t_CZST/T2_LD)**2))  
    p_anc3 = 0.5*(1. - math.exp(-(0.1*t_CZST/T2_ST)**2))  
    p_data4 = 0.5*(1. - math.exp(-(0.01*t_CZST/T2_LD)**2))  
    p_anc4 = 0.5*(1. - math.exp(-(0.01*t_CZST/T2_ST)**2))  
    p_data0 = 0.5*(1. - math.exp(-(0.*t_CZST/T2_LD)**2))  
    p_anc0 = 0.5*(1. - math.exp(-(0.*t_CZST/T2_ST)**2))  

    p_ImeasST2 = 0.5*(1. - math.exp(-(total_meas_t/T2_LD)**2))
    p_ImeasST3 = 0.5*(1. - math.exp(-(0.1*total_meas_t/T2_LD)**2))
    p_ImeasST4 = 0.5*(1. - math.exp(-(0.01*total_meas_t/T2_LD)**2))
    p_ImeasST0 = 0.5*(1. - math.exp(-(0.*total_meas_t/T2_LD)**2))

    
    ps_STa = [p_prepST, p_measST, p_1qST, p_CZST]
    
    ps_ST2b = [p_IprepST, p_data2, p_anc2, p_ImeasST2]
    ps_ST3b = [p_IprepST, p_data3, p_anc3, p_ImeasST3]
    ps_ST4b = [p_IprepST, p_data4, p_anc4, p_ImeasST4]
    ps_ST0b = [p_IprepST, p_data0, p_anc0, p_ImeasST0]
    ...
    for subset in list_errors:
        ...
        t_lower_ST2 += (t_ST*p_occurrence_ST2)
        t_lower_ST3 += (t_ST*p_occurrence_ST3)
        t_lower_ST4 += (t_ST*p_occurrence_ST4)
        t_lower_ST0 += (t_ST*p_occurrence_ST0)
    ...
    p_LDqubit_ST2_lower = 0.5*(1. - math.exp(-(t_lower_ST2/T2_LD)**2))
    p_LDqubit_ST2_upper = 0.5*(1. - math.exp(-(t_upper_ST/T2_LD)**2))
    p_LDqubit_ST3_lower = 0.5*(1.-math.exp(-(0.1*t_lower_ST3/T2_LD)**2))
    p_LDqubit_ST3_upper = 0.5*(1.-math.exp(-(0.1*t_upper_ST/T2_LD)**2))
    p_LDqubit_ST4_lower = 0.5*(1.-math.exp(-(0.01*t_lower_ST4/T2_LD)**2))
    p_LDqubit_ST4_upper = 0.5*(1.-math.exp(-(0.01*t_upper_ST/T2_LD)**2))



    output_string += '%.15f %.15f %.15f %.15f %.15f %.15f %.15f %.15f %.15f %.15f %.15f %.15f %.15f %.15f %.15f\n' %(t, p_lower_ST2, p_upper_ST2, p_lower_ST3, p_upper_ST3, p_lower_ST4, p_upper_ST4, p_lower_ST0, p_upper_ST0, p_LDqubit_ST2_lower, p_LDqubit_ST2_upper, p_LDqubit_ST3_lower, p_LDqubit_ST3_upper, p_LDqubit_ST4_lower, p_LDqubit_ST4_upper) 

data_filename = summary_folder + 'resultsQECd3_Imeas' + str(int(ratio_meas_times)) + 'T2_Tplus.dat'
